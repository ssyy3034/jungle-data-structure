...
1. 제출을 위한 준비: 제출자 전용 dir과 branch 생성
    ```sh
    $ cd Week08                              # Week08 dir로 이동
    $ mkdir ${제출자ID}                      # 제출자 전용 dir 생성
    $ cd ${제출자ID}
    $ git checkout -b Week08/${제출자ID}     # 제출자 전용 branch 생성
    $ git push -u origin Week08/${제출자ID}  # git push를 위한 사전 작업
    ```
2. 코드 제출: `Week08/${제출자ID}/` 디렉토리에 코드 복사
    - `Week08/${제출자ID}/` 디렉토리를 만들고 일주일간 만들었던 다음 파일들만 추가 (다른 파일은 제외)
...
3. 리뷰어를 위한 준비: Pull request
    - https://github.com/krafton-jungle/KJ-W05-08-<기수>-<교육장번호>.git 에서 내 branch로 제대로 제출 되었는지 확인
    - `base:main <- compare:Week08/${제출자ID}` 방향으로 pull request 생성
